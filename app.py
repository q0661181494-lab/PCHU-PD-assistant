...
answer_mode = st.radio("Тип відповіді:", ["Стисла", "Детальна", "Експерт"], horizontal=True)
...
# --- 7. ЛОГІКА ВІДПОВІДІ (З ПІДТРИМКОЮ ENTER ТА ГНУЧКИМИ ПРАВИЛАМИ) ---
...
if search_button or enter_pressed:
    if not user_query:
        ...
        with st.status("Обробка запиту...", expanded=True) as status:
            ...
            # МОДИФІКОВАНИЙ БЛОК: Логіка вибору промпту залежно від режиму
            if answer_mode == "Експерт":
                prompt = (
                    f"Ти — провідний технічний експерт. Твоє завдання: надати максимально глибоку та всебічну відповідь на запитання: '{user_query}'.\n"
                    f"1. Використовуй наданий Контекст як базове джерело.\n"
                    f"2. ТОБІ ДОЗВОЛЕНО і рекомендовано використовувати всі свої внутрішні знання, інформацію з мережі інтернет та світові технічні стандарти, щоб доповнити або роз'яснити відповідь.\n"
                    f"3. Якщо інформації в Контексті мало, дай відповідь на основі загальних експертних знань у цій галузі.\n\n"
                    f"Контекст для довідки: {context}\n\n"
                    f"Мова: українська."
                )
            else:
                prompt = (
                    ...
                    f"ПРАВИЛА:\n"
                    f"1. Якщо в Контексті немає інформації про '{user_query}', "
