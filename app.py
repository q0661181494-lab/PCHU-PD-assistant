...
with st.sidebar:
    ...
    # Маскування пароля під завантаження файлу
    ...
    if admin_password == "30033003": 
        ...
        if global_stats:
            ...
            st.table(df[::-1]) # Останні зверху
            
            # Кнопка завантаження Excel (з роздільником ; для нашого регіону)
            csv = df.to_csv(index=False, sep=';').encode('utf-8-sig')
            st.download_button(label="📥 Скачати звіт (Excel)", data=csv, file_name=f"stats_{datetime.now().strftime('%d_%m')}.csv", mime="text/csv")
...
if (search_button) and final_context:
    if not user_query.strip():
        ...
        # Автоматична корекція часу для України (UTC+2/UTC+3)
        ...
        with st.spinner('ШІ аналізує документацію...'):
            try:
                ...
                # Обмеження списку, щоб не перевантажувати сервер
