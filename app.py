...
def get_ai_response(prompt):
    """Отримує відповідь від ШІ, перебираючи ваші 5 ключів"""
...
# CSS для широких кнопок та карток
st.markdown("""
    <style>
...
# Вибір файлу та режиму
available_files = sorted([f for f in os.listdir(".") if f.endswith(".pdf")])
if not available_files:
    st.error("Завантажте PDF-інструкції в репозиторій!")
    st.stop()
...
# Пошуковий запит
user_query = st.text_input("Пошук", placeholder="Наприклад: яка ширина колії у кривих?", key="query_field", label_visibility="collapsed")
    ...
    if not user_query:
        ...
        with st.status("Обробка запиту...", expanded=True) as status:
            ...
            if answer:
                ...
                status.update(label="❌ Помилка з'єднання з ШІ", state="error")
